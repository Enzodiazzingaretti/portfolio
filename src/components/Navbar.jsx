...
/**
 * Scrollspy: devuelve el id de la sección más visible entre los hrefs internos.
 * Mismo criterio de "mejor ratio" que usa GlobalFX para la phase del fondo.
 */
function useActiveSection(internalLinks) {
  const [active, setActive] = useState("");
  const ids = useMemo(
    () => internalLinks.map((item) => item.href.slice(1)),
    [internalLinks],
  );
...
export default function Navbar({ nav, languages, currentLanguage, onLanguageChange, scrolled, navVisible }) {
  ...
  const internalLinks = useMemo(() => nav.filter((i) => !i.external), [nav]);
  const externalLinks = nav.filter((i) => i.external);
  const activeSection = useActiveSection(internalLinks);
    ...
    <header className={`fixed left-0 top-0 z-50 w-full px-4 pt-4 transition-all duration-500 md:px-8 ${navVisible ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"}`}>
      ...
      <nav className={`mx-auto hidden max-w-fit items-center border border-white/[0.07] backdrop-blur-xl transition-all duration-500 md:flex ${scrolled ? "bg-ink/85 shadow-xl shadow-black/40" : "bg-ink/25"}`}>
        {internalLinks.map((item, idx) => {
          ...
          return (
            <a
              ...
              className={`nav-link relative whitespace-nowrap px-3 py-[11px] font-mono text-label uppercase tracking-[0.26em] transition-colors duration-200 hover:text-white lg:px-5 ${
                isActive ? "nav-link--active text-white" : "text-white/60"
              }`}
            >
              {item.label}
        ...
        {externalLinks.map((item) => (
          <span key={item.href} className="hidden items-center lg:flex">
            <span className="h-3 w-px bg-white/[0.07]" />
            <a
              href={item.href}
              target="_blank"
              rel="noreferrer"
              className="whitespace-nowrap px-3 py-[11px] font-mono text-label uppercase tracking-[0.26em] text-raveRed/80 transition-colors duration-200 hover:text-raveRed lg:px-5"
            >
              {item.label} ↗
            </a>
          </span>
          ...
          {internalLinks.map((item) => {
            ...
            return (
              <a
                ...
                aria-current={isActive ? "location" : undefined}
                className={`flex items-center justify-between border-b border-white/[0.05] py-3 font-mono text-caption uppercase tracking-[0.28em] transition hover:text-white ${
                  isActive ? "text-white" : "text-white/65"
                }`}
              >
                {item.label}
                {isActive && <span className="h-1 w-1 rounded-full bg-raveRedBright" />}
              </a>
            );
          })}
          {externalLinks.map((item) => (
